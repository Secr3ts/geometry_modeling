...
enum MENU { MENU_CATMULLCLARK, MENU_DRAWWIREFRAME, MENU_EXIT, MENU_DRAWMESH, MENU_LOOP, MENU_DRAWMESHVERTICES,
	...
	MENU_REVOLUTION, MENU_CHECKMESH // Added
};
...
bool warning = true; // if true display warnings on top of bottom left informations
int duration = 10; // cycles
...
void menu(int item)
{
	...
	case MENU_REVOLUTION:
	{
		...
		profile.push_back(myPoint3D(0.0, -0.2, 0.0));
		profile.push_back(myPoint3D(0.1, -0.2, 0.0));
		profile.push_back(myPoint3D(0.3, 0.0, 0.0));
		profile.push_back(myPoint3D(0.4, 0.3, 0.0));
		profile.push_back(myPoint3D(0.35, 0.5, 0.0));
		profile.push_back(myPoint3D(0.1, 0.55, 0.0));

		m->revolution(12, 360, profile);
		clear();
		m->computeNormals();
		makeBuffers(m);
		break;
	...
	case MENU_SIMPLIFY:
		{
			if (!m->triangulated())
			{
				warning = true;
				duration = fps * 5;
				cout << "Mesh is not triangulated." << std::endl;
				break;
			}

			m->shortestEdgeCollapse();
			m->computeNormals();
			m->checkMesh();
			makeBuffers(m);
			break;
...
void display() 
{
	...
	if (drawsilhouette)
	{
		...
		for (vector<myHalfedge *>::iterator it = m->halfedges.begin(); it != m->halfedges.end(); it++)
		{
			...
			if (f1 && f2)
			{
				myVector3D v1_cameraEye = myVector3D(
					camera_eye.X - v1->normal->dX, 
					camera_eye.Y - v1->normal->dY, 
					camera_eye.Z - v1->normal->dZ
				);
				
				double d1 = f1->normal->dX * v1_cameraEye.dX + f1->normal->dY * v1_cameraEye.dY + f1->normal->dZ * v1_cameraEye.dZ;
				double d2 = f2->normal->dX * v1_cameraEye.dX + f2->normal->dY * v1_cameraEye.dY + f2->normal->dZ * v1_cameraEye.dZ;
			
				valid = d1 * d2 <= 0.0;
			}
			
			if ( valid /*ADD THE CONDITION TO CHECK IF THE HALFEDGE DEFINED BY (V1, V2) IS A SILHOUETTE EDGE*/ )
			{
	...
	if (duration > 0 && warning)
	{
		draw_text(0.0f, 60.0f, 0, "Mesh is not triangulated.", color);
		duration--;
	}
